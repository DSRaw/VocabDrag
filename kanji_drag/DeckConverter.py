...
def browse_filesystem():
    ...
    max_chars = entry_len/curObj.entry_font.measure("0")                        #Calculates the maximum number of characters than can fit in pathEntry without going off screen

    deck_path = tkFileDialog.askopenfilename(filetypes=[("Text", "*.txt")])     #The path to be used for file operations
    file_path = deck_path                                                       #Allows path to be used in reseize function when it is called from DeckConverterGUI
    
    if (len(deck_path) <= 0):
        curObj.file_path.set("No file selected.")
        print "File not selected, or path invalid."    
    else:
        if (len(deck_path) > max_chars):                                               #Truncates file path if too long for min-size window
            clean_path = _shortify(deck_path, entry_len, max_chars)
...
#The following function will dynamically recalculate how much of the file path to display to fit the width of the user's window on resize
def resize_path_display(widget):
    global curObj
    global file_path
    entry_len = curObj.pathEntry.winfo_width()
    max_chars = entry_len/curObj.entry_font.measure("0")
    
    
    #print max_chars
    
    if (len(file_path) > max_chars):
        curObj.file_path.set(_shortify(file_path, entry_len, max_chars))
...
def _shortify(file_path, entry_len, char_len):
    ...
    if (parts_len + len(str(short_path)) <= max_chars):
        return file_path
    
    else:
        if (len(parts_list) > 0):
            parts_list.pop()                                                        #Removes root directory from list since that is customly added in short_path line
